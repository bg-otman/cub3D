...
void	draw_tex(t_texture tex, int x, t_data *data)
{
	while (tex.first < tex.last)
	{
		...
		put_pixel_to_buffer(data->buffer, x, tex.first, get_pixel_color(tex.img, tex.x, tex.y));
...
void	draw_textures(t_data *data, t_texture tex, int x, double angle)
{
	...
	tex.strip = (TILE_SIZE / tex.ray->wall_dist) * ((WIN_WIDTH / 2) / tan(FOV / 2));
	...
	if ((tex.ray->side == 0 && tex.ray->ray_dir_x < 0)
		|| (tex.ray->side == 1 && tex.ray->ray_dir_y > 0))
		tex.x = tex.img->width - tex.x - 1;
	tex.pos = (tex.first - WIN_HEIGHT/2 + tex.strip / 2) * (tex.img->height / tex.strip);
